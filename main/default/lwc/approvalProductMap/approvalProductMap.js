...
import TARGET_OBJECT_ID from '@salesforce/schema/ProcessInstance.TargetObjectId';
import PRODUCT_LOOKUP from '@salesforce/schema/Brand_Review__c.Product__c';
import getProductName from '@salesforce/apex/ApprovalProcessController.getProductName';
import getTargetObjectId from '@salesforce/apex/ApprovalProcessController.getTargetObjectId';
...
export default class ApprovalProductMap extends LightningElement {
    ...
    @wire(getTargetObjectId, {workItemId: '$recordId'})
    wiredTargetObjectId({ error, data }) {
        if (data) {
            this.targetObjectId = data;
            // console.log(this.targetObjectId);
            
        } else if (error) {
            console.error('Error fetching ProcessInstance:', error);
        }
    }

    @wire(getProductName, { targetObjectId: '$targetObjectId'})
    wiredProductName({data,error}){
        if(data){
            console.log(data);
